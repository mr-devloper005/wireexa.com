...
import { ArrowRight, ArrowUpRight, Bookmark, BriefcaseBusiness, Building2, Camera, Download, FileText, Filter, Image as ImageIcon, MapPin, Megaphone, Search, Newspaper, UserRound } from 'lucide-react'
...
import { RailPostCard } from '@/editable/cards/PostCards'
...
export async function EditableTaskArchiveRoute({
  ...
  const resolvedBasePath = basePath || taskConfig?.route || `/${task}`
  let { posts, pagination } = await fetchPaginatedTaskPosts(task, { page, limit: 24, category })

  // Real-data fallback: the task-scoped feed returns empty when master posts are untyped.
  // Re-derive from the unscoped feed via getPostTaskKey and paginate manually. Never mock.
  if (!posts.length) {
    try {
      ...
      const all = (feed?.posts || []).filter((post) => {
        ...
        return category === 'all' || normalizeCategory(postCategory) === category
      })
      if (all.length) {
        const limit = 24
        const totalPages = Math.max(1, Math.ceil(all.length / limit))
        const safePage = Math.min(page, totalPages)
        posts = all.slice((safePage - 1) * limit, safePage * limit)
        pagination = { page: safePage, limit, total: all.length, totalPages, hasPrevPage: safePage > 1, hasNextPage: safePage < totalPages }
      }
    } catch {
      // keep empty state
    }
  ...
  return <TaskArchiveView task={task} posts={posts} pagination={pagination} category={category} basePath={resolvedBasePath} />
}
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archiveVars = { '--archive-bg': 'var(--slot4-page-bg)', '--archive-text': 'var(--slot4-page-text)', '--archive-surface': 'var(--slot4-surface-bg)', '--archive-accent': 'var(--slot4-accent)' } as CSSProperties
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        ...
        <section className="relative overflow-hidden border-b border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]">
          <div className="pointer-events-none absolute -right-24 -top-24 h-72 w-72 rounded-full bg-[var(--slot4-accent)]/10 blur-3xl wx-float" />
          <div className="mx-auto max-w-[var(--editable-container)] px-5 py-14 sm:px-6 lg:px-8 lg:py-20">
            <span className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-[var(--slot4-accent-strong)]">
              <Newspaper className="h-3.5 w-3.5" /> The newsroom
            </span>
            <h1 className="mt-6 max-w-4xl text-4xl font-black leading-[1.04] tracking-[-0.035em] sm:text-6xl">
              {category === 'all' ? label : categoryLabel}
            </h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-[var(--slot4-muted-text)]">
              Press releases, announcements, and media-ready stories — organized for fast discovery and continuous coverage.
            </p>
            <p className="mt-6 text-xs font-bold uppercase tracking-[0.2em] text-[var(--slot4-soft-muted-text)]">{total} {total === 1 ? 'release' : 'releases'} published</p>
          </div>
        </section>

        {/* Filter rail */}
        <section className="sticky top-[80px] z-20 border-b border-[var(--editable-border)] bg-[var(--slot4-page-bg)]/90 backdrop-blur">
          <div className="mx-auto flex max-w-[var(--editable-container)] items-center gap-2 overflow-x-auto px-5 py-4 sm:px-6 lg:px-8 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <Filter className="h-4 w-4 shrink-0 text-[var(--slot4-soft-muted-text)]" />
            <Link href={basePath} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold transition ${category === 'all' ? 'bg-[var(--slot4-accent)] text-white' : 'border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] text-[var(--slot4-muted-text)] hover:border-[var(--slot4-accent)]'}`}>All</Link>
            {categories.slice(0, 10).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={`shrink-0 whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold transition ${category === item.slug ? 'bg-[var(--slot4-accent)] text-white' : 'border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] text-[var(--slot4-muted-text)] hover:border-[var(--slot4-accent)]'}`}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-5 py-14 sm:px-6 lg:px-8 lg:py-16">
          {lead ? (
            <Link href={`${basePath}/${lead.slug}`} data-reveal className="wx-on-dark group relative mb-8 block overflow-hidden rounded-[2rem] bg-[var(--slot4-dark-bg)] p-8 text-white sm:p-12">
              <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-[var(--slot4-accent)]/30 blur-3xl wx-float" />
              <div className="relative">
                <span className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-white/80">Lead release · {getCategory(lead, label)}</span>
                <h2 className="mt-6 max-w-3xl text-3xl font-black leading-[1.05] tracking-[-0.03em] sm:text-5xl">{lead.title}</h2>
          ...
          {rest.length ? (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {rest.map((post, index) => (
                <RailPostCard key={post.id || post.slug} post={post} href={`${basePath}/${post.slug}`} index={index} />
              ))}
            </div>
          ) : !lead ? (
            <div className="rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-14 text-center">
              <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]"><Search className="h-6 w-6" /></div>
              <h2 className="mt-5 text-2xl font-black tracking-[-0.03em]">No releases found</h2>
              <p className="mt-3 text-sm text-[var(--slot4-muted-text)]">Try another category, or check back as new releases are published to the wire.</p>
              <Link href={basePath} className="mt-6 inline-flex items-center gap-2 rounded-full bg-[var(--slot4-accent)] px-5 py-3 text-sm font-bold text-white">Browse all releases</Link>
            </div>
          ) : null}

          {pagination.totalPages && pagination.totalPages > 1 ? (
            <div className="mt-12 flex items-center justify-center gap-3">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-bold transition hover:border-[var(--slot4-accent)]">Previous</Link> : null}
              <span className="rounded-full bg-[var(--slot4-accent)] px-5 py-3 text-sm font-bold text-white">Page {page} of {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 py-3 text-sm font-bold transition hover:border-[var(--slot4-accent)]">Next</Link> : null}
            </div>
          ) : null}
        </section>
      </main>
